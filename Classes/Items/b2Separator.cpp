...
void b2Separator::Separate(b2Body* pBody, b2FixtureDef* pFixtureDef, std::vector<b2Vec2>* pVerticesVec, int scale) {
...
void b2Separator::calcShapes(std::vector<b2Vec2> &pVerticesVec, std::vector<std::vector<b2Vec2> > &result) {
    std::vector<b2Vec2> vec;
    int i, n, j,minLen;
    float d, t, dx, dy;
    int i1, i2, i3;
    ...
    while (!queue.empty()) {
        ...
        for (i=0; i<n; i++) {
            ...
            if ((d<0)) {
                ...
                vec1 = new std::vector<b2Vec2>();
                vec2 = new std::vector<b2Vec2>();
...
bool b2Separator::hitSegment(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
    float t1 = x3-x1;
    float t2 = y3-y1;
    float t3 = x2-x1;
    float t4 = y2-y1;
